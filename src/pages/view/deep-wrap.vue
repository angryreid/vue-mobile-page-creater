...
export default {
  ...
  methods: {
    ...
    showLog(data) {
      console.group("View 收到消息 " + new Date().toLocaleTimeString());
      console.log(data);
      console.groupEnd();
    },
    ...
    delDeepIndex: function(arr, key, prevKey = "") {
      return arr.filter((item, index) => {
        let currentKey =
          prevKey !== "" ? prevKey + "-" + index : index.toString();
        if (currentKey === key) return false;
        if (item.child && item.child.length > 0)
          item.child = this.delDeepIndex(item.child, key, currentKey);
        return true;
      });
    },
    ...
    changeDeepIndex: function(arr, key, res, prevKey = "") {
      return arr.map((item, index) => {
        ...
        if (item.child)
          item.child = this.changeDeepIndex(item.child, key, res, currentKey);
        return item;
      });
    }
  }
